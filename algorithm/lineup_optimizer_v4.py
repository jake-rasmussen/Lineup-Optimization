...
def parse_and_optimize_lineup(json_input, excel_file_path, method='exhaustive', max_iterations=1000, top_n=15):
    ...
    Args:
        ...
        top_n (int): Number of top lineups to consider for leadoff optimization
    ...
    if os.path.exists(bdnrp_csv) and os.path.exists("leadoff_brp.csv") and os.path.exists("second_ab_brp.csv"):
        ...
        print("Loaded pre-calculated BDNRP data and leadoff values from CSV files.")
    else:
        # Generate all data
    ...
    optimizer.top_n = top_n  # Set the number of top lineups to track
    
    # Set leadoff and second at-bat BRP data
    ...
    # Step 5: Calculate the unweighted expected run production with the leadoff-adjusted lineup
    lineup_stats = optimizer.get_lineup_stats(leadoff_lineup)
    base_score = lineup_stats['base_bdnrp'].sum()
    
    # Step 6: Format the result as a JSON object
    result = {}
    ...
    # Add information about the leadoff selection
...
if __name__ == "__main__":
    ...
    test_input = {
        ...
        "method": "exhaustive",
        "max_iterations": 1000,
        "top_n": 15  # Number of top lineup cycles to evaluate for leadoff optimization
    }
    ...
    for pos, player in [(pos, result[pos]) for pos in map(str, range(1, 10))]:
        print(f"{pos}. {player}")
