...
def parse_and_optimize_lineup(json_input, excel_file_path, method='exhaustive', max_iterations=1000, top_n=15):
    ...
    # Check if position 1 is constrained (a player is explicitly assigned to the leadoff spot)
    position_1_constrained = False
    position_1_player = None
    
    if "1" in json_input and json_input["1"] is not None and json_input["1"]["name"] != "" and json_input["1"]["data"] is not None:
        position_1_constrained = True
        position_1_player = json_input["1"]["name"]
        print(f"\nPosition 1 (leadoff) is constrained to player: {position_1_player}")
...
if __name__ == "__main__":
  ...
  "top_n": 15
    ...
    if not result['leadoff_info']['constrained']:
        print(f"Original cycle rank: {result['leadoff_info']['original_cycle_rank']} of {result['leadoff_info']['total_cycles_evaluated']}")